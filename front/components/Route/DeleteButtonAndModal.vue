              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-slate-500">
                    Are you sure you want to delete the route "<span
                      class="font-semibold text-slate-600">{{ props.routeName }}</span>"?
                    <br/>
                    The route will be permanently removed.
                    <br/>
...
const emit = defineEmits(["route-deleted"]);
const httpClient = useRoutesHttpClient();

const modalOpened: Ref<boolean> = ref(false);

async function delete_() {
    await httpClient.delete_(props.routeId!);
    closeModal();
    emit("route-deleted");
}
